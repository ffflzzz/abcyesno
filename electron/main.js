...
// ── App window (launcher openMode:"window") ───────────────────────────────
// Mirrors createDetachedPanelWindow but loads the FULL App in studio mode
// (ChatLayout + StudioWorkbench), not just the bare ResultPanel. Triggered
// by a launcher icon that declares `launcher.openMode: "window"` (currently
// manjucraft_agent / 「漫剧go」). The new window shares the same backend
// (AG-UI bridge + Hermes gateway) as the main window.
//
// De-dup key `app={workflowId}` so re-clicking the same icon focuses the
// existing window instead of spawning a second one.
function resolveManifestName(workflowId) {
  try {
    const mf = path.join(
      HERMES_FORK, 'skills', 'langgraph_agents', 'agents', workflowId, 'manifest.json'
    );
    if (fs.existsSync(mf)) {
...
function createAppWindow(opts = {}) {
  ...
  const title = `Abcyesno · ${resolveManifestName(workflowId)}`;
  const params = new URLSearchParams({
    panel: 'studio',
    workflow: workflowId,
  });
  ...
  win.webContents.on('render-process-gone', (_e, details) => {
    log('main', `app-window render-process-gone: reason=${details.reason} exitCode=${details.exitCode}`);
  });
  win.webContents.on('console-message', (_e, level, message, line, sourceId) => {
    const levelName = ['verbose', 'info', 'warning', 'error'][level] || level;
    log('app-window', `[${levelName}] ${message} (${sourceId}:${line})`);
  });
  win.on('closed', () => allWindows.delete(win));
  allWindows.add(win);
  win.loadURL(url);
...
ipcMain.handle('open-app-window', (_event, opts) => {
  const workflowId = opts?.workflowId || '';
  // De-dup: same workflowId → focus existing window instead of spawning a new one.
  const target = `app=${workflowId}`;
  for (const w of allWindows) {
    if (w.__appWindowKey === target) {
      if (w.isMinimized()) w.restore();
      w.focus();
      return { success: true, reused: true };
    }
  }
  const win = createAppWindow({ workflowId });
  win.__appWindowKey = target;
  return { success: true, reused: false };
});

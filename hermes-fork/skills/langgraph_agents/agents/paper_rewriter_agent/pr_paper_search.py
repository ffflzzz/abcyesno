"""搜索并下载论文工具（多源版本）— vendored from paper_rewriter_langgraph.
...
与源项目的差异：_RUNS_DIR 来自 pr_config（HERMES_HOME 下），不再依赖项目根目录。
文本提取优先 pymupdf，其次 pdfminer；都没有则返回 PDF 路径由用户手动处理。
"""
...
from pr_config import RUNS_DIR as _RUNS_DIR

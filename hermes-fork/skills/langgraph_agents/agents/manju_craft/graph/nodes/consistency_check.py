...
def _score_similarity(path_a: str, path_b: str) -> float:
    ...
    try:
        ...
        return 0.85
    # imagehash.__sub__ returns a numpy scalar; coerce to a plain Python int
    # so the score stays JSON/msgpack-serializable (the LangGraph checkpointer
    # used for HITL interrupt() must serialize the full state).
    distance = int(hash_a - hash_b)
    # Heuristic: distance 0 => 1.0, distance >= 20 => 0.0

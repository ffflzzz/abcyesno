...
@method("session.set_yolo")
def _(rid, params: dict) -> dict:
    # Toggle the session approval bypass (yolo). Frontend "default" sends
    # enabled=False (restore backend "ask"); "yolo" sends enabled=True.
    session, err = _sess(params, rid)
    if err:
        return err
    ...
    try:
        ...
        return _err(rid, 5000, f"yolo toggle failed: {e}")
    agent = session.get("agent")
    if agent is not None:
        try:
            _emit(
            ...
            pass
    return _ok(rid, {"status": "yolo_on" if enabled else "yolo_off", "enabled": enabled})

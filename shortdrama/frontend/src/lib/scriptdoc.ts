/* ==========================================================================
   src/lib/scriptdoc.ts —— 剧本正文 → 结构化 HTML（只读展示用）
   --------------------------------------------------------------------------
   从 `web/assets/js/core/scriptdoc.js` **逐条移植**，解析纪律一条没动。

   为什么需要它：`scriptwriter` 的产物是 markdown，直接丢进 <textarea> 给人看，
   界面上就是 `> 系列：…`、`**场景：**`、`#### 镜头 3（近景，7s）` 这种源码 ——
   用户原话（2026-09-18）：「可读性很差，页面 html 混乱」。
...
/**
 * 场次行：markdown 的 `### 第N场…`、中文编号的 `第三场`、粘贴型 `第一场内景 老屋 - 日`、
 * `1.内景 便利店 - 夜`。
 * ⚠️ **必须锚在"场次"这个形态上**，不能宽泛到"标题里含『场』" ——
 *    实测 `## 场次列表` 会被那种写法误判成场次行（当年自测当场抓到）。
 */
...
const RE_BULLET = /^[-*+]\s+(.*)$/;
/**
 * ★★ 尖括号在模型产物里**不是 HTML**，有两种用法（真实产物全量扫出 91 处）：
...
export type Block =
  | { t: 'blank' } | { t: 'hr' }
  | { t: 'h'; lvl: number; role: 'scene' | 'shot' | 'plain'; text: string }
  | { t: 'scene' | 'shot' | 'note' | 'tag' | 'action' | 'li' | 'meta' | 'p'; text: string }
  | { t: 'kv'; k: string; v: string }
  | { t: 'speaker'; name: string; tone: string }
  | { t: 'line'; text: string };
 ...
 *   hr / p  普通段落
 */
export function parse(md: unknown): Block[] {
  const out: Block[] = [];
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (s.length <= 12 && !RE_SENT_END.test(s) && !/^[#>*|]/.test(s)) {
      out.push({ t: 'speaker', name: s, tone: '' });
      continue;
    }
    out.push({ t: 'p', text: s });
  }
...
export function toHtml(blocks: Block[]): string {
  ...
  const flushMeta = () => {
    ...
    parts.push('<div class="sd-meta">'
      + meta.map((x) => '<div class="sd-meta-row">' + inline(x) + '</div>').join('')
      + '</div>');
    meta = [];
  };
  const flushLis = () => {
    ...
    parts.push('<ul class="sd-ul">'
      + lis.map((x) => '<li>' + inline(x) + '</li>').join('') + '</ul>');
    lis = [];
  };
  const flushAction = () => {
  ...
  const flushAll = () => { flushMeta(); flushLis(); flushAction(); };
  ...
  blocks.forEach((b) => {
    ...
    switch (b.t) {
      ...
      default: parts.push('<p class="sd-p">' + inline((b as { text: string }).text) + '</p>'); return;
    }
  });
  flushAll();
  return parts.join('');
...
export interface ScriptStats {
  blocks: number; scenes: number; shots: number; lines: number;
  speakers: number; notes: number; paragraphs: number;
}

/** 统计（给测试与排障看：**解析结果要可核对**，不能只报"渲染了"）。 */
export function stats(blocks: Block[]): ScriptStats {
  const c: ScriptStats = {
    blocks: blocks.length, scenes: 0, shots: 0, lines: 0, speakers: 0, notes: 0, paragraphs: 0,
  };
  blocks.forEach((b) => {
 ...
 */
export function render(md: unknown): { html: string; stats: ScriptStats; blocks: Block[] } {
  const blocks = parse(md);
  return { html: toHtml(blocks), stats: stats(blocks), blocks };
}

export const ScriptDoc = { render, parse, toHtml, stats, esc };

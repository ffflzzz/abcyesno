...
def bind(root: Path, shots: list[dict], max_n: int = 5,
    ...
    **location 类只在宽景绑**（2026-09-23 收窄；2026-09-09 原为「一律不绑」）：
    场景图自带固定机位，会覆盖分镜的景别/机位。LN01 是「店内全景」，只因正文
    提到"透过玻璃门洒入"就绑了「玻璃门入口」（店外视角图）→ 静帧直接画成站在
    店门外。故现按景别分档：**全景 / 远景 / 大全景 / 空镜**才绑场景图（构图本来
    就是 Wide、不打架，且空镜正是场景漂移的重灾区），中近景与特写不绑、靠文本锚点。
    """
    reg = auto_sync(root)
    prot = protagonist(root, reg)
    ...
    for s in shots:
        ...
        #   所以宽景才绑它；中近景仍只靠**文本锚点** —— 见 `assets.scene_lines` 与
        ...
        #   本镜角色数 2  → 封顶 3（= 2 角色图 + 1 道具图；场景要走 `scene_pick`
        #                   并**占一个道具位**，见下方 2026-09-23 那段）
        #
